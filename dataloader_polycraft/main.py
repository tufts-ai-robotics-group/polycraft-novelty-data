...
def create_data_generators(batch_size = 1, shuffle = True, classes_to_include = None):
    ...
    noe = 3 #Numer of environments
    n_p = 32 #Patch size, patch --> n_p x n_p
    scale = 0.5 #Scale factor, use 1 for original scale
    novelty = 'novel_height' #'novel_item', 'novel_height', 'normal'
...
if __name__ == '__main__':
    ...
    train_dataloader, valid_dataloader, test_dataloader = create_data_generators(batch_size = 1, shuffle = True, classes_to_include = None)
    ...
    for i, sample in enumerate(train_dataloader):#loop over dataset
        ...
        train_img = patches1[0,0,0,: :,:].numpy()
