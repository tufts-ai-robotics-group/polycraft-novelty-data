...
from utils import get_active_windows, get_paths, sleep_and_display


class PolycraftInterface:
    def __init__(self, host, port, reset_command, render_bool=False, save_json=False):
        # Given attributes:
        self.host = host
        self.port = port
        ...
        if self.save_json:
            ...
            csv_filename = 'env_state_detailed' + ".csv"
    ...
    def run_SENSE_INVENTORY_and_update(self, inventory_output_inventory=None):
        ...
        for slot in inventory_output_inventory:
            self.inventory_quantity_dict[inventory_output_inventory[slot]['item']] = inventory_output_inventory[slot][
                'count']
    ...
    def run_SENSE_ALL_and_update(self, parameter=None, set_agent_id=False):
        ...
        if parameter == None:
            ...
            for location, entity in self.entities.items():
                ...
                self.map_to_plot[y][x] = self.items_id[entity['item']]
    ...
    def execute_action(self, action):
        ...
        if action.split(' ')[0].upper() in self.move_commands:
            ...
            craft_command_result, sense_inventory_command_result = self.run_CRAFT_and_update_inventory(action)
