# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 13:59:39 2021
...
import polycraft_nov_data.dataloader_itemname as mydataloader
...
def create_data_generators(batch_size=1, shuffle=True, classes_to_include=None):
    ...
    total_noi_i = 300  # Total number of processed images from one environemnt i
    noe = 1  # Numer of environments, currently we only have one dataset for every item
    n_p = 32  # Patch size, patch --> n_p x n_p
    scale = 0.25  # Scale factor, use 1 for original scale
    novelty = 'novel_item'  # Novel item since we want to use specific new items
    ...
    for i in range(noe):
        dataset_env_i = mydataloader.PolycraftDataset(nov_type=novelty, noi=total_noi_i,
                                                      env_idx=i, p_size=n_p, scale_factor=scale,
                                                      item_name=classes_to_include)
...
if __name__ == '__main__':
    ...
    train_dataloader, valid_dataloader, test_dataloader = create_data_generators(batch_size=1,
                                                                                 shuffle=True, classes_to_include='minecraft:tnt')
